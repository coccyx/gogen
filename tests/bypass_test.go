...
func TestAccountingBypass(t *testing.T) {
	// Test that max performance mode (CacheIntervals = 2147483647) bypasses accounting
	config := &internal.Config{
		Global: internal.Global{
			CacheIntervals: 2147483647, // max performance mode - bypass accounting
			ROTInterval:    1,
		},
	}
	...
	case <-time.After(100 * time.Millisecond):
		t.Fatal("Account() blocked when it should have bypassed in max performance mode")
	}
	...
	case <-time.After(100 * time.Millisecond):
		t.Fatal("ReadFinal() blocked when it should have bypassed in max performance mode")
	}
}
